...
전 세계의 90% 이상의 클라이언트 PC를 지배하고 있는 거대한 제국, 마이크로소프트의 윈도우(Windows). 
하지만 진지한 컴퓨터 공학과 시스템 파이프라인의 시선에서 들여다본다면 윈도우는 그저 화려한 마우스 클릭 화면 껍데기가 아닙니다.

이번 1주차 렉처에서는 1980년대 검은 도스(MS-DOS) 프롬프트 위에서 알파벳 `C:\`가 왜 우리의 하드디스크 드라이브 표준으로 각인되었는지, 그리고 그 불안정했던 껍데기를 박살 내고 기업용 시장을 장악한 **강철 아키텍처 NT 커널**이 어떤 철학으로 설계되어 리눅스와 견줄 만한 현대적 운영체제의 근간이 되었는지 심해의 역사를 역추적합니다.

---

## 📚 하위 문서 목차 (Sub-Chapters)

단순한 프로그램 개발사를 넘어 하드웨어 제어 철학의 변천 과정을 따라가는 핵심 2부작입니다.

1. **[MS-DOS와 C:\ 경로의 기원](./01_ms_dos/index.md)**
   > 윈도우 키+R `cmd`를 치면 나오는 콘솔 화면의 실체. A와 B 플로피 시대를 지나 하드디스크 C 드라이브에 정착한 역사적 기원.
2. **[NT 커널과 진정한 OS 아키텍처](./02_nt_kernel/index.md)**
   > 커널 모드와 유저 모드가 나뉘지 않아 시스템 셧다운이 일상이던 DOS/Win98 시절을 파괴하고, 프로세스의 완벽한 하드웨어 격리를 이뤄낸 신기술(New Technology) 커널 엔진.

<hr style="margin: 40px 0;">

> **💡 개발자를 위한 조언**
> "자신이 매일 코드를 작성해 띄워 올리는 윈도우가 어떤 시대를 거쳐 지금의 파일 스펙을 만들었는지 모른 채 개발된 툴 위에서만 코딩하는 것은, 엑셀 칸 채우기와 다를 바 없습니다."
