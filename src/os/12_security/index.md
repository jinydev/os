...
# 12주차: 현대 OS 보안 계층 (Linux 권한·SELinux·eBPF)

![OS Core Architecture](/Users/hojin/.gemini/antigravity/brain/28d2e8ff-2bf4-4b06-8f22-23880f1f7300/ai_os_12.png)
<br>

서버가 구동되고 가상화 인프라를 마련했다면, 마지막 과제는 외부와 내부로부터의 악의적 침공에 맞서는 단단한 방패, **보안(Security)** 계층 아키텍처 디자인입니다.

이번 12주차에서는 일반 서버 기초 관리 권한인 rwx 소유주 배분 체계(DAC) 위에 숨겨진 SetUID 권한 탈취 방어선부터 시작합니다. 나아가 루트(Root) 마저도 정책 위반 시 찢어버려 서버 붕괴를 막는 NSA 주도의 MAC(강제 제어 규칙) 기반 **SELinux**의 위엄을 살펴봅니다. 
종국에는 커널 심부에 컴파일 리스크 없이 안전한 코드를 찔러 넣어 해킹 통신이 발생하기 전 실시간으로 모가지를 잘라버리는 최신 사이버 킬-체인(Kill-Chain), **eBPF와 seccomp 패러다임**을 디코딩합니다.
...
> **💡 보안 아키텍처의 패러다임 변화**
> "과거의 보안이 성벽을 둘러싸고 바깥의 적을 막는 방화벽(Firewall)의 성격이었다면, 현대 클라우드의 보안 통제망은 모든 적이 이미 내성(Network/Kernel) 안에 들어왔다는 가정하에 수만 가닥의 모니터링 JIT 센서(eBPF)를 혈관마다 박아넣는 무결성 제로 트러스트(Zero Trust)이다."
